...
- (AFHTTPRequestOperation *)createRequest:(NSString *)url method:(NSString *)method tag:(NSInteger)tag
    ...
    if (tempError != nil) {
        NSLog(@"******************************请求开始(%@)******************************",@(tag));
        NSLog(@"请求地址:%@",url);
        NSLog(@"请求出错:%@", tempError.localizedDescription);
        NSLog(@"******************************请求完成(%@)******************************\n\n\n",@(tag));
        if (error) {
...
- (void)handleRequest:(AFHTTPRequestOperation *)httpOperation
    ...
    NSLog(@"******************************请求开始(%@)******************************",tag);
    NSLog(@"请求地址:%@",httpOperation.request.URL);
    NSLog(@"请求头信息:%@",httpOperation.request.allHTTPHeaderFields);
    NSLog(@"请求内容:%@",[body stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding]);
    [httpOperation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        [[UIApplication sharedApplication] setNetworkActivityIndicatorVisible:NO];
        NSLog(@"响应头信息:%@",operation.response.allHeaderFields);
        NSLog(@"响应内容:%@", operation.responseString);
        NSLog(@"请求用时:%f",[[NSDate date] timeIntervalSinceDate:startDate]);
        NSLog(@"******************************请求完成(%@)******************************\n\n\n",tag);
        if (success) {
        ...
        NSLog(@"响应头信息:%@",operation.response.allHeaderFields);
        NSLog(@"响应内容:%@", operation.responseString);
        NSLog(@"响应错误:%@", error.localizedDescription);
        NSLog(@"请求用时:%f",[[NSDate date] timeIntervalSinceDate:startDate]);
        NSLog(@"******************************请求完成(%@)******************************\n\n\n",tag);
        if (failure) {
